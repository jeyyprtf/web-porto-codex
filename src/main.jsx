...
function App() {
  ...
  const goTo = (page) => {
    setActivePage(page); setMenuOpen(false); window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  ...
  return <>
    <header className="site-header">
      <button className="wordmark" onClick={() => goTo('home')} aria-label="Mara Aster home"><span>MA</span><em>MARΑ ASTER</em></button>
    ...
    <footer className="footer">
      <div className="footer-top"><div><p className="eyebrow">Have a vision?</p><button className="footer-link" onClick={() => goTo('contact')}>Let's make it <i>visible.</i> <ArrowUpRight /></button></div><div className="footer-mark">MA<span>© 2024</span></div></div>
...
function Home({ goTo, works, openWork }) {
  return <div className="page home-page">
    <section className="hero section-pad">
      ...
      <div className="hero-image-wrap reveal"><img src="https://images.unsplash.com/photo-1488426862026-3ee34a7d66df?auto=format&fit=crop&w=1800&q=90" alt="Woman in a red dress standing in a sunlit field" /><div className="image-stamp">SCROLL TO EXPLORE <ArrowDownRight size={13} /></div></div>
      <div className="hero-side-note">01 / <span>Light is the subject.<br />Everything else is a feeling.</span></div>
    </section>
    <section className="manifesto section-pad"><p className="eyebrow">A little about the work</p><p className="manifesto-text">The work lives somewhere between <i>instinct</i> and intention — built on honest connection, a love for natural light and the belief that the best frame is often the one you didn't plan.</p><button className="text-link" onClick={() => goTo('about')}>More about Mara <ArrowRight size={15} /></button></section>
...
function About({ goTo }) {
  return <div className="page about-page"><section className="about-hero section-pad"><div><p className="eyebrow">About / The person behind the frame</p><h1>Hi, I'm <i>Mara.</i></h1><p className="about-lead">A photographer and visual director drawn to the honest, the tactile and the beautifully unfinished.</p></div><div className="about-hero-image"><img src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=1200&q=85" alt="Mara, a portrait photographer" /><span>Self portrait<br />Ubud, 2024</span></div></section><section className="about-story section-pad"><div className="story-label"><p className="eyebrow">My point of view</p><span>02—04</span></div><div className="story-copy"><p>There is a quiet kind of magic in paying attention. The sun moving across a wall. The split second before someone smiles. Hands finding their way through a familiar ritual.</p><p>That's where I like to work — in the in-between moments that reveal something true. My process is collaborative, calm and a little bit curious. I want you to feel seen, never performed.</p><button className="text-link" onClick={() => goTo('contact')}>Work together <ArrowRight size={15} /></button></div></section><section className="about-details section-pad"><div><p className="eyebrow">The details</p><h2>Good things<br />take <i>time.</i></h2></div><div className="detail-list"><div><span>01</span><b>Based in</b><p>Bali, Indonesia<br />Available worldwide</p></div><div><span>02</span><b>Specialising in</b><p>Portraits · Editorial<br />Brand stories · Film</p></div><div><span>03</span><b>Published in</b><p>Vogue Living · Kinfolk<br />Cereal · The Gentlewoman</p></div></div></section></div>;
}
